...
const LeagueClientAPI = require('./Components/LCUModule.js');
const LOL = new LeagueClientAPI();
 ...
 * Add event listeners...
 */
 LOL.on('connect', () => console.log('Found lockfile'))
 ...
 .on('event', ev => {
   ...
   if(ev.uri === '/lol-champ-select-legacy/v1/session'){
     if(ev.data && ev.data.actions){
      sendMessage('success') 
      //  counterFinder(ev.data.actions[0][0].championId)
      }
    }
})
